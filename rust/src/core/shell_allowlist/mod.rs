...
fn rewrite_case_constructs(command: &str) -> Result<String, ShellError> {
...
fn parse_case_construct(input: &str) -> Result<(String, usize), ShellError> {
...
fn find_case_pattern_end(input: &str, start: usize) -> Option<usize> {
    let bytes = input.as_bytes();
    let mut i = start;
...
fn find_case_arm_end(input: &str, start: usize) -> Option<CaseArmEnd> {
    let bytes = input.as_bytes();
    let mut i = start;
    let mut paren_depth = 0u32;
    let mut brace_depth = 0u32;
...
fn find_shell_word(input: &str, word: &str, start: usize) -> Option<usize> {
    let bytes = input.as_bytes();
...
fn is_shell_word_at(input: &str, start: usize, word: &str) -> bool {
...
fn is_shell_word_boundary(bytes: &[u8], start: usize, word_len: usize) -> bool {
    let boundary = |c: u8| {
...
fn skip_shell_whitespace(input: &str, start: usize) -> usize {
    input
        .as_bytes()
        .iter()
        .enumerate()
...
fn skip_shell_expansion(bytes: &[u8], start: usize) -> Option<usize> {
...
fn case_parse_error(input: &str, reason: &str) -> ShellError {
    format!(
